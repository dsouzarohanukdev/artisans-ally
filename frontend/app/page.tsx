...
export default function Home() {
    ...
    const [activeTab, setActiveTab] = useState<'analysis' | 'workshop'>('workshop');
    const [activeAnalysisTab, setActiveAnalysisTab] = useState<'etsy' | 'ebay'>('etsy');

    const fetchWorkshopData = async () => {
        setIsWorkshopLoading(true);
        try {
            const response = await fetch('http://127.0.0.1:5000/api/workshop');
            if (!response.ok) throw new Error("Failed to fetch workshop data");
            const data = await response.json();
            setWorkshopData(data);
        } catch (err) {
            console.error(err); setError("Could not load workshop data.");
        } finally { setIsWorkshopLoading(false); }
    };

    useEffect(() => { fetchWorkshopData(); }, []);
    ...
    const handleDeleteMaterial = async (materialId: number) => {
        ...
        try {
            const response = await fetch(`http://127.0.0.1:5000/api/materials/${materialId}`, { method: 'DELETE' });
    ...
    const handleDeleteProduct = async (productId: number) => {
        ...
        try {
            const response = await fetch(`http://127.0.0.1:5000/api/products/${productId}`, { method: 'DELETE' });
    ...
    const sortedEtsyListings = [...etsyListings].sort((a, b) => 
        (a.price.amount / a.price.divisor) - (b.price.amount / b.price.divisor)
    );
    const sortedEbayListings = [...ebayListings].sort((a, b) => 
        (a.price.amount / a.price.divisor) - (b.price.amount / b.price.divisor)
    );
    
    return (
        <main className="font-sans container mx-auto p-4 md:p-8 bg-gray-50 min-h-screen">
            <header className="text-center mb-12"><h1 className="text-4xl md:text-5xl font-bold text-gray-800">Artisan's Ally</h1><p className="text-lg md:text-xl text-gray-500 mt-2">Business Dashboard for Makers</p></header>
            ...
            <div className="max-w-2xl mx-auto bg-white p-6 rounded-lg shadow-md mb-12">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                    ...
                    <div><label htmlFor="productSelect" className="block text-sm font-medium text-gray-700">2. Select Your Product</label><select id="productSelect" value={selectedProductId} onChange={(e) => setSelectedProductId(e.target.value)} className="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500"><option value="">-- Or enter cost manually below --</option>{workshopData.products.map(p => (<option key={p.id} value={p.id}>{p.name} (COGS: £{p.cogs?.toFixed(2)})</option>))}</select></div>
                </div>
                <div>
                    <label htmlFor="materialCost" className="block text-sm font-medium text-gray-700">3. Material Cost (£)</label>
                    ...
                    <p className="text-xs text-gray-500 mt-1">This is your 'Cost of Goods Sold' (COGS). For an accurate number, go to the <button onClick={() => setActiveTab('workshop')} className="font-semibold text-blue-600 hover:underline">Workshop Manager</button> to create a product recipe.</p>
                </div>
            ...
            <div className="max-w-5xl mx-auto">
                <div className="border-b border-gray-200">
                    <nav className="-mb-px flex gap-6" aria-label="Tabs">
                        ...
                        <button onClick={() => setActiveTab('workshop')} className={`shrink-0 border-b-2 px-1 pb-4 text-sm font-medium ${activeTab === 'workshop' ? 'border-indigo-500 text-indigo-600' : 'border-transparent text-gray-500 hover:border-gray-300 hover:text-gray-700'}`}>Workshop Manager</button>
                    </nav>
                </div>
                <div className="mt-8">
                    {activeTab === 'analysis' && (
                        <div>
                            {overallAnalysis ? (
                                <div className="space-y-12">
                                    <div>
                                        <div className="text-center mb-8"><h2 className="text-3xl font-bold text-gray-800">Your Profit Scenarios</h2><p className="text-md text-gray-500 mt-2">Calculated using your cost of <span className="font-semibold">£{parseFloat(materialCost).toFixed(2)}</span> against the overall market average of <span className="font-semibold">£{overallAnalysis.average_price.toFixed(2)}</span>.</p></div>
                                        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">{scenarios.map((s)=>(<div key={s.name} className="bg-white p-6 rounded-lg shadow-md border text-center"><h3 className="font-semibold text-gray-800">{s.name}</h3><p className="text-sm text-gray-500 mb-2">Set Price at £{s.price.toFixed(2)}</p><p className="text-2xl font-bold text-green-600">£{s.profit.toFixed(2)}</p><p className="text-sm text-gray-500">Estimated Profit</p></div>))}</div>
                                    </div>
                                    <div>
                                        <div className="border-b border-gray-200">
                                        ...
                                        <div className="mt-8">
                                            {activeAnalysisTab === 'etsy' && etsyAnalysis && (
                                                <div className="space-y-8">
                                                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-center">
                                                        <div className="bg-white p-4 rounded-lg shadow-sm border"><p className="text-sm text-gray-500">Listings Found</p><p className="text-xl font-bold text-gray-800">{etsyAnalysis.count}</p></div>
                                                        <div className="bg-white p-4 rounded-lg shadow-sm border"><p className="text-sm text-gray-500">Average Price</p><p className="text-xl font-bold text-orange-600">£{etsyAnalysis.average_price.toFixed(2)}</p></div>
                                                        <div className="bg-white p-4 rounded-lg shadow-sm border"><p className="text-sm text-gray-500">Price Range</p><p className="text-xl font-bold text-gray-800">£{etsyAnalysis.min_price.toFixed(2)} - £{etsyAnalysis.max_price.toFixed(2)}</p></div>
                                                    </div>
                                            ...
                                            {activeAnalysisTab === 'ebay' && ebayAnalysis && (
                                                <div className="space-y-8">
                                                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-center">
                                                        <div className="bg-white p-4 rounded-lg shadow-sm border"><p className="text-sm text-gray-500">Listings Found</p><p className="text-xl font-bold text-gray-800">{ebayAnalysis.count}</p></div>
                                                        <div className="bg-white p-4 rounded-lg shadow-sm border"><p className="text-sm text-gray-500">Average Price</p><p className="text-xl font-bold text-blue-600">£{ebayAnalysis.average_price.toFixed(2)}</p></div>
                                                        <div className="bg-white p-4 rounded-lg shadow-sm border"><p className="text-sm text-gray-500">Price Range</p><p className="text-xl font-bold text-gray-800">£{ebayAnalysis.min_price.toFixed(2)} - £{ebayAnalysis.max_price.toFixed(2)}</p></div>
                                                    </div>
                        ...
                        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                            <div className="space-y-6">
                                ...
                                <div className="bg-white p-6 rounded-lg shadow-md">
                                    ...
                                    <p className="text-sm text-gray-500 mb-4">Enter a raw material you've purchased. The app will calculate the cost per gram, ml, or unit for you.</p>
                                    <form onSubmit={handleAddMaterial} className="space-y-4">
                                        ...
                                        <div className="grid grid-cols-3 gap-4">
                                            <input type="number" placeholder="Cost (£)" step="0.01" value={newMaterial.cost} onChange={e=>setNewMaterial({...newMaterial,cost:e.target.value})} required className="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500"/>
                                            <input type="number" placeholder="Total Qty" value={newMaterial.quantity} onChange={e=>setNewMaterial({...newMaterial,quantity:e.target.value})} required className="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500"/>
                                            <select value={newMaterial.unit} onChange={e=>setNewMaterial({...newMaterial,unit:e.target.value})} className="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500"><option value="g">grams (g)</option><option value="kg">kilograms (kg)</option><option value="ml">milliliters (ml)</option><option value="L">litres (L)</option><option value="unit">units</option><option value="pieces">pieces</option></select>
                                        </div>
                                ...
                                {newProductRecipe.map((item, index) => {
                                    ...
                                    return (
                                        <div key={index} className="flex gap-2 items-center">
                                            ...
                                            <span className="w-1/4 text-sm text-gray-500 pl-1">{selectedMaterial?.unit || 'unit'}</span>
