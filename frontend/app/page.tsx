                    ...
                    {isAuthLoading ? ( <p className="text-center text-gray-500 py-8">Loading user session...</p> ) : !user ? (
                        ...
                        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                                        ...
                                        {workshopData.products.map(p => (
                                            <div key={p.id} className="bg-white p-5 rounded-lg shadow-md border relative">
                                                ...
                                                <div className="mt-2 text-sm text-gray-600">
                                                    <div className="flex justify-between"><span>Material Cost (COGS):</span> <span className="font-medium">{currencySymbol}{p.material_cost.toFixed(2)}</span></div>
                                                    <div className="flex justify-between"><span>Labour ({p.labour_hours} hr @ {currencySymbol}{p.hourly_rate}/hr):</span> <span className="font-medium">{currencySymbol}{p.labour_cost.toFixed(2)}</span></div>
                                                </div>
                                                <div className="mt-2 pt-2 border-t flex justify-between text-gray-900">
                                                    <span className="font-bold">Total Cost to Make:</span>
                                                    <span className="font-bold">{currencySymbol}{p.total_cost.toFixed(2)}</span>
                                                </div>
                                                <div className="mt-2 pt-2 border-t flex justify-between text-blue-600">
                                                    ...
                                                    <span className="font-bold text-lg">{currencySymbol}{p.suggested_price.toFixed(2)}</span>
                                                </div>
                                            </div>
                                        ))}
                                    </div>
                            ...
                            <div className="space-y-6">
                                ...
                                {isWorkshopLoading ? <p className="text-center py-4">Loading...</p> : workshopData.materials.length === 0 ? (<p className="text-center text-gray-500 py-4">Start by adding your raw materials here first. This will be used in adding the new product.</p>) : (
                                    <div className="bg-white p-4 rounded-lg shadow-md"><ul className="divide-y divide-gray-200">{workshopData.materials.map(m=>(
                                        <li key={m.id} className="py-3 flex justify-between items-center gap-4 text-sm">
                                            <div className="flex-1"><p className="font-semibold text-gray-800">{m.name}</p><p className="text-gray-500">{currencySymbol}{m.cost.toFixed(2)} for {m.quantity} {m.unit} = <span className="font-semibold text-gray-700">{currencySymbol}{m.cost_per_unit?.toFixed(4)} per {m.unit}</span></p></div>
                ...
                <section className="mb-12">
                    ...
                    <div className="max-w-2xl mx-auto bg-white p-6 rounded-lg shadow-md">
                        <div className="grid grid-cols-1 gap-4 mb-4">
                                        ...
                                        <ul className="divide-y divide-gray-200">
                                            {filteredProducts.map(product => (
                                                ...
                                                >
                                                    ...
                                                    <p className="text-sm text-gray-600">Your Cost: {currencySymbol}{product.total_cost.toFixed(2)}</p>
                                                </li>
                                            ))}
                                        </ul>
                                    </div>
                            ...
                            <div>
                                <label htmlFor="marketplace" className="block text-sm font-medium text-gray-700">2. Select Marketplace</label>
                                <select 
                                    id="marketplace" 
                                    value={marketplace} 
                                    ...
                                    className="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500"
                                >
                                    <option value="EBAY_GB">eBay UK ({currencySymbol === '£' ? '£' : 'GBP'})</option>
                                    <option value="EBAY_US">eBay US ({currencySymbol === '$' ? '$' : 'USD'})</option>
                                    <option value="EBAY_AU">eBay Australia ({currencySymbol === '$' ? '$' : 'AUD'})</option>
                                    <option value="EBAY_DE">eBay Germany ({currencySymbol === '€' ? '€' : 'EUR'})</option>
                                    <option value="EBAY_FR">eBay France ({currencySymbol === '€' ? '€' : 'EUR'})</option>
                                    <option value="EBAY_IT">eBay Italy ({currencySymbol === '€' ? '€' : 'EUR'})</option>
                                    <option value="EBAY_ES">eBay Spain ({currencySymbol === '€' ? '€' : 'EUR'})</option>
                                </select>
                            </div>

                            <div>
                                <label htmlFor="totalCost" className="block text-sm font-medium text-gray-700">3. Your Total Cost to Make ({currencySymbol})</label>
                                <input 
                                    type="number" 
                                    id="totalCost" 
                                    value={totalCost} 
                    ...
                    <div className="max-w-5xl mx-auto space-y-12">
                        ...
                        <section>
                            <div className="text-center mb-8">
                                ...
                                <p className="text-md text-gray-500 mt-2">Calculated using your total cost of <span className="font-semibold">{currencySymbol}{parseFloat(totalCost).toFixed(2)}</span> against the market average.</p>
                            </div>
                            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                                {scenarios.map((s)=>(
                                    <div key={s.name} className="bg-white p-6 rounded-lg shadow-md border text-center">
                                        ...
                                        <p className="text-3xl font-bold text-gray-900 my-2">{currencySymbol}{s.price.toFixed(2)}</p>
                                        
                                        <p className={`text-xl font-semibold ${s.profit >= 0 ? 'text-green-600' : 'text-red-600'}`}>
                                            {s.profit >= 0 ? `${currencySymbol}${s.profit.toFixed(2)}` : `-${currencySymbol}${Math.abs(s.profit).toFixed(2)}`}
                                        </p>
                            ...
                            <div className="mt-8">
                                {activeAnalysisTab === 'ebay' && ebayAnalysis && (
                                    <div className="space-y-8">
                                        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-center"><div className="bg-white p-4 rounded-lg shadow-sm border"><p className="text-sm text-gray-500">Listings Found</p><p className="text-xl font-bold text-gray-800">{ebayAnalysis.count}</p></div><div className="bg-white p-4 rounded-lg shadow-sm border"><p className="text-sm text-gray-500">Average Price</p><p className="text-xl font-bold text-blue-600">{currencySymbol}{ebayAnalysis.average_price.toFixed(2)}</p></div><div className="bg-white p-4 rounded-lg shadow-sm border"><p className="text-sm text-gray-500">Price Range</p><p className="text-xl font-bold text-gray-800">{currencySymbol}{ebayAnalysis.min_price.toFixed(2)} - {currencySymbol}{ebayAnalysis.max_price.toFixed(2)}</p></div></div>
                                        <div>
                        ...
                        <div className="bg-white p-8 rounded-lg shadow-2xl w-full max-w-lg">
                            ...
                            <form onSubmit={handleMaterialSubmit} className="space-y-4">
                                <div>
                                    ...
                                    <input type="text" id="materialName" placeholder="e.g., Jesmonite AC100 Powder" value={materialForm.name} onChange={e=>setMaterialForm({...materialForm, name: e.target.value})} required className="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500"/>
                                </div>
                                <div className="grid grid-cols-3 gap-4">
                                    <div>
                                        <label htmlFor="materialCost" className="block text-sm font-medium text-gray-700">Total Cost ({currencySymbol})</label>
                                        <input type="number" id="materialCost" placeholder="Total Cost" step="0.01" value={materialForm.cost} onChange={e=>setMaterialForm({...materialForm, cost: e.target.value})} required className="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500"/>
                                    </div>
                                    <div>
                                        <label htmlFor="materialQty" className="block text-sm font-medium text-gray-700">Total Qty</label>
                                        <input type="number" id="materialQty" placeholder="e.g., 5000" value={materialForm.quantity} onChange={e=>setMaterialForm({...materialForm, quantity: e.target.value})} required className="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500"/>
                                    </div>
                                    <div>
                                        <label htmlFor="materialUnit" className="block text-sm font-medium text-gray-700">Unit</label>
                                        <select id="materialUnit" value={materialForm.unit} onChange={e=>setMaterialForm({...materialForm, unit: e.target.value})} className="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500"><option value="g">grams (g)</option><option value="ml">milliliters (ml)</option><option value="unit">units</option><option value="pieces">pieces</option></select>
                                    </div>
                                </div>
                                ...
                                <div className="space-y-6">
                                    ...
                                    <div>
                                        ...
                                        <div className="flex gap-4">
                                            ...
                                            <div className="w-1/2">
                                                <label htmlFor="hourlyRate" className="block text-sm font-medium text-gray-700">Rate per Hour ({currencySymbol})</label>
