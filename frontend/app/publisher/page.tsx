...
export default function PublisherPage() {
    ...
    const handleEbayAuth = async () => {
        ...
        try {
            const response = await fetch('http://127.0.0.1:5000/api/ebay/get-auth-url', { credentials: 'include' });
    ...
    const handleSubmit = async (e: FormEvent) => {
        ...
        try {
            const response = await fetch('http://127.0.0.1:5000/api/ebay/create-draft', {
                method: 'POST',
            ...
            if (!response.ok) {
                // Try to parse a helpful error message from eBay's response
            ...
            <>
                ...
                <main className="font-sans container mx-auto p-4 md:p-8 bg-gray-50 min-h-screen">
                    <div className="text-center text-gray-500 py-8 px-4 bg-white rounded-lg shadow-md">
                        ...
                        <p className="mt-2">Please <Link href="/login" className="text-indigo-600 font-semibold hover:underline">log in</Link> or <Link href="/register" className="text-indigo-600 font-semibold hover:underline">register</Link> to get started.</p>
                    </div>
                </main>
            </>
        );
        ...
        <>
            ...
            <main className="font-sans container mx-auto p-4 md:p-8 bg-gray-50 min-h-screen">
                <header className="text-center mb-8">
                    ...
                    <p className="text-lg md:text-xl text-gray-500 mt-2">Create a master listing and publish it to eBay.</p>
                </header>
                    ...
                    {user.has_ebay_token ? (
                        <div>
                            ...
                            <form onSubmit={handleSubmit} className="space-y-4">
                                <div><label htmlFor="title" className="block text-sm font-medium text-gray-700">Title</label><input type="text" id="title" value={title} onChange={(e) => setTitle(e.target.value)} required placeholder="e.g., Handmade Jesmonite Tray" className="mt-1 w-full p-2 border border-gray-300 rounded-md"/></div>
                                <div><label htmlFor="description" className="block text-sm font-medium text-gray-700">Description</label><textarea id="description" rows={5} value={description} onChange={(e) => setDescription(e.target.value)} required placeholder="Describe your beautiful product..." className="mt-1 w-full p-2 border border-gray-300 rounded-md"></textarea></div>
                                <div><label htmlFor="price" className="block text-sm font-medium text-gray-700">Price (£)</label><input type="number" id="price" value={price} onChange={(e) => setPrice(e.target.value)} required step="0.01" placeholder="e.g., 25.00" className="mt-1 w-full p-2 border border-gray-300 rounded-md"/></div>
